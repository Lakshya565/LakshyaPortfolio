...
A visitor may use the project scene to discover work across categories. Conventional navigation and project lists remain visible so the scene never becomes a comprehension test.
...
- Gives every public project a stable, shareable, indexable URL.
...
Semantic headings, links, project rows, and routes form the reliable base. Motion and the map enhance that base when the device and user preferences permit.
...
| Homepage | Animated 2D project environment | Accepted | It must remain an enhancement over ordinary navigation. |
...
| Map placement | Typed frontend configuration keyed by slug | Accepted | Geometry is presentation data and should evolve with the circuit scene. |
...
| Scene direction | Circuit/computer-system environment | Accepted as the base concept | The exact composition will be prototyped and iterated. |
...
The global navigation exposes Work, About, GitHub, LinkedIn, and Email. External links need clear labels and safe new-tab behavior. Mobile users must not need to interact with the map to find contact information.

### Working homepage order

1. Identity and animated project environment.
2. Featured work led by Cisco and RepoFrame.
3. Remaining featured projects, with Cisco employment context contained within its case study rather than repeated as a separate block.
4. Supporting project evidence and media.
5. Lower-priority archive preview.
6. About and personal-background preview.
7. Contact calls to action.
...
The site may still use client-side JavaScript for navigation enhancement, the circuit scene, galleries, and motion. Those interactions must not be required to obtain the underlying project links or case-study content.
...
- Validates unique slugs, record shapes, link schemes, asset paths, alt text, map placement, section identifiers, and launch completeness.
...
- Keeps map/motion code replaceable and independent of case-study content.
...
Exact folders may change in Phase 0. Keep the repository single-application unless a real second deployable appears. Content loading, validation, visual components, map placement, and motion remain separate responsibilities without manufacturing empty abstraction layers.
...
`lib/map/placements.ts` maps stable project slugs to normalized desktop/mobile positions, visual tier, optional connection groups, and safe accent tokens. The content manifest determines eligibility; the placement file determines geometry. Validation fails when a published map-visible project lacks a placement.
...
- Map placement coverage and normalized coordinate bounds.
...
Use server components for content-heavy route composition and metadata. Use client components only for animation, responsive scene state, galleries, menus, and other browser-only interaction. This keeps initial HTML meaningful and limits JavaScript.
...
## Interactive map and motion

### Map behavior

Use SVG for scalable geometry and semantic DOM links or buttons for nodes. Every node needs a visible label, keyboard focus, adequate target size, and normal URL destination.

The scene consumes published map-eligible project summaries, typed placements, and constrained accent tokens. Mobile should not simply shrink the desktop scene: a scrollable constellation, simplified path, or conventional list may be better. Prototype the fallback before committing to one.

### Base circuit-scene concept

The first prototype should resemble a stylized computer motherboard or system diagram rather than a literal geographic map:

- Lakshya's name and headline form the central processor/identity core.
- Featured projects appear as the largest connected modules, with Cisco and RepoFrame on the primary high-bandwidth traces.
- Supporting projects appear as peripheral boards, sensors, or expansion modules grouped loosely by software, AI, and hardware—not placed in rigid career silos.
- Animated green and purple traces carry restrained pulses between nodes, suggesting information and power flow.
- Hover/focus expands a node into a readable label and short description without requiring zoom.
- Selecting a node sends a short trace pulse toward it and navigates immediately to the case study.
- The background may use subtle grid, solder-pad, via, and waveform motifs without imitating a real commercial board.
- Mobile uses a simplified vertical “signal path” with the same project hierarchy rather than a miniature board.

This is a base design, not a locked composition. Early prototypes should test whether it communicates engineering breadth or instead becomes visual noise. The circuit metaphor must support the content hierarchy rather than forcing every project into an artificial hardware analogy.
...
- Placement coverage for every map-visible project.
...
- Semantic links underlying project nodes.
- Non-map work navigation on the same page.
...
- Schema/build tests for slugs, publication state, manifest-to-MDX linkage, asset paths, and map placement.
- Component tests for semantics and keyboard behavior.
- Route tests for minimal, maximal, draft-excluded, and 404 states.
- Focused end-to-end coverage for direct URLs, history, map selection, reduced motion, and mobile fallback.
...
- The 2D scene enhances discovery and has a complete fallback.
...
### Animation before visual language is risky

Building the full map before typography, spacing, contrast, and layout primitives creates rework. The phased plan establishes a minimal visual system before the animation, then performs final polish later.
...
### Circuit placement must remain presentation data

Coordinates are not project facts. Keeping them in a typed placement module makes circuit redesign an isolated frontend change.
...
- Store circuit-scene placement in typed frontend configuration.
...
- Begin the interactive environment from an atmospheric sci-fi circuit/computer-system metaphor and iterate.

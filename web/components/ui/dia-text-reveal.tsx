...
/*
 * Vendored from the Magic UI registry (`npx shadcn add @magicui/dia-text-reveal`).
 *
 * `react-hooks/refs` is disabled for this file rather than fixed. The component
 * writes `optsRef.current` and `playRef.current` during render — the standard
 * latest-value ref pattern — so that the animation loop, which only ever runs
 * from an effect or a timeout, reads current props without restarting the
 * sweep. Rewriting it would mean re-deriving that timing, and the next
 * `shadcn add` of this component would silently overwrite the change anyway.
 *
 * Keep local edits out of this file. Customise it through props from the call
 * site instead; see `app/page.tsx`.
 */
